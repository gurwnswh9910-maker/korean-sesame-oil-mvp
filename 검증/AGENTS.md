...
- 고의도 채널 KPI와 24h 이후 실행 게이트는 `검증/고의도_채널_KPI_및_다음실험_20260628.md`와 `mvp/post_24h_action_packet_20260628.md`를 본다. note 1~5 합산 views가 30 미만이고 응답 0이면 유통 실패로 보고 6차 note를 바로 게시하지 않는다. 2026-06-28T21:56+09:00 실제 24h gate는 note 1~5 합산 views 36, comments 0, スキ 0, 실제 응답 0이라 `problem_language_or_cta_failure_consider_aromaloss_note`다. 이는 검증 성공이 아니라 최소 유통은 됐는데 문제 언어/CTA가 응답을 만들지 못했다는 판정이다. 6차 note는 `/aroma-loss-goma`, `/quick-answer`, `/field-aroma` pre-publish routing 확인 후에만 게시 후보로 본다. 유통 실패 후 외부 채널 선택은 `mvp/post_gate_external_channel_packet_20260628.md`를 보며, X/Threads URL 없는 공개 질문 1회, 오프라인 신오쿠보/한국식품점 인터뷰, Konest 규칙 확인 중 1개만 선택한다. note 외부 실제 게시/댓글은 별도 명시 허용 전 하지 않는다.
...
- 개봉 후 향 손실/큰 병 부담 검증은 `https://korean-sesame-oil-mvp.vercel.app/aroma-loss-goma`와 source `content_aromaloss`를 사용한다. note 원고는 `mvp/note_aromaloss_posting_packet.md`에 준비됐지만 아직 live note 게시 전이다. 24h gate 이후에는 바로 게시가 아니라 6차 note 라우팅/CTA 검수 후 게시 후보로 둔다.
