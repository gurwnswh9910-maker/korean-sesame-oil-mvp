...
- note dashboard views/comments/スキ는 `scripts/record_note_dashboard_snapshot.py`로 `experiments/note_dashboard_snapshots.csv`에 기록한다. 먼저 `--dry-run`으로 숫자 파싱을 확인한 뒤 실제 기록하고, `scripts/summarize_validation_signals.py`를 재실행한다. 2026-06-28T16:56+09:00부터 `--from-dashboard-text .\.tmp\note_dashboard_current.txt --fill-missing-zero`로 대시보드 본문 텍스트를 자동 파싱할 수 있다. 단, `aggregation_at`이 24h 체크 기준보다 오래되면 기록하지 않는다.
