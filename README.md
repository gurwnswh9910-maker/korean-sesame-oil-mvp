...
- `research/08_micro_jtbd_and_competing_solutions_20260628.md`: Kadoya 5g使い切りパック까지 포함한 작은 사용 장면 JTBD와 경쟁 대체재 보강
- `검증/참기름_팔기_욕구_세분화_20260628.md`: `참기름 팔기`를 사용량-향 보존, 대체재 선택 피로, 搾りたて 체험, 재구매 불편 등으로 세분화한 최신 KB
- `검증/향미오일_포지셔닝_가설_20260628.md`: 한국식 방앗간 참기름을 `炒め油`가 아니라 EVOO 같은 `仕上げの香味油`로 이해할 수 있는지 보는 최신 가설
...
- `experiments/note_comment_candidate_status.csv`: note 댓글 후보들의 공개/API 댓글 가능 상태 snapshot
- `experiments/note_comment_execution_queue.csv`: note 댓글 후보 6개의 문제 적합도 기준 실행 순서
- `scripts/record_note_dashboard_snapshot.py`: note 대시보드 snapshot을 CSV에 기록하는 helper
- `scripts/check_note_public_api.py`: note 공개 API에서 게시 상태, 댓글 수, スキ 수를 확인하는 helper. 기본값은 MVP note 1~5이며, `--note SOURCE=URL_OR_KEY`를 반복하면 외부 note 댓글 후보도 확인할 수 있다. 조회수는 대시보드 snapshot으로만 기록한다.
- `scripts/check_note_comment_candidates.py`: `experiments/channel_posting_log.csv`의 `note external comment candidate` 행을 읽어 댓글 후보가 아직 `published`/`can_comment`인지 재확인하고, `--write`로 후보 상태 CSV를 갱신한다.
...
- Aroma-loss/finishing-oil flyer PDF: https://korean-sesame-oil-mvp.vercel.app/field-aroma-flyer
...
```powershell
$env:PYTHONUTF8='1'; $env:PYTHONIOENCODING='utf-8'; python .\scripts\record_public_social_response.py --platform X --source x_threads_travel --url "https://x.com/{user}/status/{id}" --text "{공개 답글 요약}" --recent-purchase "{최근 구매/탐색}" --brand-store "{브랜드/店名}" --volume-price "{용량/가격}" --use-up-period "{1병 사용 기간}" --aroma-memory "{향 기억/향 저하}" --substitute-comparison "{기존 대체재로 충분한지}" --needed-proof "{필요 증거}" --single-price "{100ml 가격 반응}" --sample-signal "{샘플/구매 신호}"
```

note 댓글 후보 상태는 아래처럼 재확인한다. 이 명령은 댓글을 게시하지 않고 공개 API 상태만 기록한다.

```powershell
$env:PYTHONUTF8='1'; $env:PYTHONIOENCODING='utf-8'; python .\scripts\check_note_comment_candidates.py --write
```

댓글 실제 실행 순서는 `검증/note_댓글_실행_큐_20260628.md`와 `experiments/note_comment_execution_queue.csv`를 따른다. 첫 실행은 Ottogi 110ml 대체재 후보와 신오쿠보 구매 맥락 후보 중 1~2개만, 링크 없이 진행한다.
