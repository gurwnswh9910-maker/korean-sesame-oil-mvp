...
- note account-side posting: 2026-06-27T21:03+09:00 Edge CDP check reached `https://editor.note.com/new` after login, but `POST /api/v1/text_notes` returned `422 email_activation`; note home shows a visible `メールを再送信` button, so note posting is held until the registered email verification is completed by the user
