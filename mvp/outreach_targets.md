...
| 순위 | 채널 | 이유 | 게시 문안 |
|---:|---|---|---|
| 1 | note `#韓国料理` / `#韓国旅行` / `#新大久保` / `#おうち韓国` 태그 | note 1~5차 게시 완료. 24시간/72시간 전체뷰, 댓글, スキ, 폼 응답을 각각 확인한다. 6차 `note_aromaloss` 후보는 24h 재확인 전 게시하지 않는다. | note 1~5 URL은 위 목록과 `검증/출처_목록.md`를 본다. source는 `note_kfood`, `note_content_travel`, `note_content_shinokubo`, `note_content_homecook`, `note_content_homecook_ricebowl`로 분리한다. |
