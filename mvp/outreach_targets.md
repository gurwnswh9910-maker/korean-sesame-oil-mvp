...
| 순위 | 채널 | 이유 | 게시 문안 |
|---:|---|---|---|
| 1 | GitHub Pages article / note `#韓国料理` 태그 | note 계정 게시 전에도 `note_article.html`은 즉시 공유 가능하다. 2026-06-27T19:22+09:00 현재 note 작성 URL은 로그인 페이지로 리다이렉트되므로, 계정이 준비되면 같은 원고를 자기 글로 발행하고 조회/댓글/スキ를 확인한다. | `note_article.html`, `note_article_draft.md`, `?src=article_github` / `?src=note_kfood` |
