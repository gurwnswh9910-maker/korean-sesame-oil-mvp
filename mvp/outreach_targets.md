...
Vercel과 note로 공개한 MVP에 실제 일본어 트래픽을 넣고, Notion Form 또는 댓글/DM으로 행동 신호를 모은다.
...
note posts: 1차 https://note.com/dreamy_viola8978/n/n77fa3f5a7fe9 / 2차 https://note.com/dreamy_viola8978/n/n3f3af286cf6d / 3차 https://note.com/dreamy_viola8978/n/n700b325ba824 / 4차 https://note.com/dreamy_viola8978/n/n08bad3dce2a9 / 5차 https://note.com/dreamy_viola8978/n/nbb21605544ca / 6차 https://note.com/dreamy_viola8978/n/n363650c8697f
Primary Form: https://www.notion.so/forms/38c5da6ea39c8149beb3000c9ab0ea98

채널별 링크는 `?src=`를 붙인다. 랜딩/콘텐츠 MVP는 1줄 응답 메모의 `流入元` 값을 source로 바꾸고, Notion Form URL에도 source를 넘긴다. GitHub fallback은 이 값을 Issue 제목의 `[src:...]`에 넣어 집계한다.
...
| 순위 | 채널 | 이유 | 게시 문안 |
|---:|---|---|---|
| 1 | note `#韓国料理` / `#韓国旅行` / `#新大久保` / `#おうち韓国` 태그 | note 1~6차 게시 완료. 전체뷰, 댓글, スキ, 폼 응답을 각각 확인한다. 6차는 향 손실/仕上げの香味油 질문이며 게시 직후 응답 0이다. | note 1~6 URL은 위 목록과 `검증/출처_목록.md`를 본다. source는 `note_kfood`, `note_content_travel`, `note_content_shinokubo`, `note_content_homecook`, `note_content_homecook_ricebowl`, `note_content_aromaloss`로 분리한다. |
...
| 5a | 신오쿠보/한국식품점 향 손실 수동 인터뷰 | note 무응답 이후 가장 높은 의도 채널이다. 이미 장보는 사람에게 현재 병의 용량/가격, 개봉 후 기간, 남은 병의 향, 100ml 1,480円 조건을 바로 묻는다. | `mvp/field_aroma_interview_script.md`, `/field-aroma`, `?src=offline_aromaloss` |
| 5b | 매대 앞 30초 체크 페이지 | 이미 한국 참기름 또는 향이 강한 일본 참기름을 보고 있는 사람에게 후보 상품/가격/용량/안 사는 이유를 바로 묻는다. 소량보다 제조일·압착·마지막 향 보존·재구매 편의가 전환 이유인지 확인한다. | `/shelf-check`, `/shelf`, `?src=content_shelfcheck` |
| 5c | 사용량-향 보존 인터뷰 | `참기름 팔기`를 사용량 문제로 좁힌다. 200g 이상 병을 몇 개월에 쓰는지, 1~2개월 안에 다 못 쓰는지, 기존 Kuki/Kadoya/Ottogi/신오쿠보 搾りたて 대체재로 충분한지를 묻는다. | `검증/참기름_팔기_욕구_세분화_20260628.md`, `/aroma-loss-goma`, `/shelf-check` |
...
1. note 1~5차 게시물의 24시간 반응을 분리해 본다.
2. 기다리는 동안 landing과 `/korea-trip-goma`, `/aroma-loss-goma`, `/shelf-check`의 메시지를 `검증/참기름_팔기_욕구_세분화_20260628.md` 기준으로 보정한다.
3. 게시 URL을 `experiments/channel_posting_log.csv`에 기록한다.
4. 댓글/DM은 바로 판매하지 말고 최근 구매 행동을 묻는다.
5. 긍정 댓글은 `/answer-note`로 이동시키거나, 폼이 귀찮으면 note 댓글 템플릿으로 바로 응답하게 한다. 더 긴 대화는 댓글/DM에서 최근 구매 행동을 묻는다.
6. Notion Form 제출이 막히거나 source 추적이 약하면 Google Forms/Tally로 CTA를 교체한다.
7. 외부 계정 게시 전에는 `channel_rules_and_permission_gate.md`의 권한 확인을 통과한다.
