...
작성일: 2026-06-27
상태: note 공개 직전 사용자 확인 대기

## 현재 블로커

2026-06-27T21:40+09:00 기준 이메일 인증은 완료되었다. Edge CDP로 note draft `n77fa3f5a7fe9`를 열어 제목/본문을 입력하고 초안을 저장했다. 공개 설정은 무료 글, 댓글 허용, 자동 번역 off, AI 학습 대가 환원 프로그램 off, 아래 5개 핵심 태그 적용 상태다.

```text
#韓国料理 #新大久保 #韓国旅行 #ごま油 #おうち韓国
```

현재 남은 블로커는 사용자 최종 확인이다. `投稿する`는 공개 게시 액션이므로 `게시해` 같은 명시 허용 전에는 누르지 않는다.
...
1. 공개 설정 화면에서 final `投稿する` 버튼이 보이는지 확인한다.
2. 사용자에게 공개 게시 허용을 받는다.
3. 허용이 있으면 `投稿する`를 누른다.
4. 게시 완료 후 URL을 캡처한다.
5. 제목은 아래 문구다.
