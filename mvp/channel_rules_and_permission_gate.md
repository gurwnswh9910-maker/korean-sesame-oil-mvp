...
| 채널 | 현재 판단 | 이유 | 실행 방식 |
|---|---|---|---|
| note | 1순위, 5개 게시 완료, 6차 후보 보류 | 자기 글로 수요 검증 취지를 설명할 수 있고, note 공식 도움말 기준 대시보드에서 글별 조회/댓글/스키를 확인할 수 있다. 1차 제품/입하, 2차 여행/사용, 3차 신오쿠보, 4차 홈쿡, 5차 한국김/TKG 글은 게시 완료. `mvp/note_aromaloss_posting_packet.md`는 6차 후보로 준비됐지만 24h 재확인 전 게시하지 않는다. 24h 합산 views가 30 미만이면 유통 실패로 보고 6차 note도 보류한다. | note 1~5의 24h/72h 조회, 댓글, 스키, 폼 응답을 기록한다. URL은 `검증/출처_목록.md`와 `검증/AGENTS.md`를 우선한다. |
